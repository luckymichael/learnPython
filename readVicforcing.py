...
df = pd.read_table(r'D:\Cloud\Dropbox\postdoc\summa\summaData\blivneh\full_data_41.21875_-116.21875', skiprows = num_skip_line)
...
col_format = 'i4',   'i4',    'i4',   'i4',  'f4',      'f4',      'f4',      'f4',      'f4',       'f4',       'f4'#,      'f4'
endian = 'little' # could be big-endian     

# read binary output file from VIC forcing generator
vic_file = "D:\\Cloud\\Dropbox\\postdoc\\summa\\summaData\\blivneh\\full_data_41.28125_-116.15625.new"
...
rootgrp = Dataset("D:\\Cloud\\Dropbox\\postdoc\\summa\\summaData\\blivneh\\test.nc", "w", format="NETCDF4_CLASSIC")
...
#%% test itertuples
for row in df1[['hour','airpres']].itertuples():
  print(row)
  print(row.hour) # but not row['hour']


vicfiles = [(1, vic_file), (2, vic_file)]  
b = pd.Panel({ 'frac' + str(row_Index) : read_vic_bin_forcing(row_File, col_names, col_format).iloc[:,4:] for row_Index, row_File in vicfiles })
b.sum(axis=0)
df1 + df1
...
df['grid16th'] = 'full_data_' + ((df.ymax + df.ymin) * 0.5).astype(str) + '_' + ((df.xmax + df.xmin) * 0.5).astype(str)


b = pd.Panel({ 'frac' + str(row.Index) : read_vic_bin_forcing(row.grid16th, col_names, col_format).iloc[:,4:] * row.frac_frac for row in grouped.get_group(17000001)[['grid16th','frac_frac']].itertuples() })


df_forcing_sum = []

xr.concat(df1,df1)
for row in grouped.get_group(17000001)[['grid16th','frac_frac']].itertuples(): # itertuples is more efficient than df.iterrows()
